...
import { sendOtp, checkPhoneNumber } from "../../services/AuthService";
...
function ForgotPasswordPage() {
  ...
  const handleSendOtp = async () => {
    if (validatePhoneNumber()) {
      try {
        // Kiểm tra sự tồn tại của số điện thoại trước khi gửi mã OTP
        const checkResponse = await checkPhoneNumber(phoneNumber);
        if (checkResponse.status === 200) {
          await sendOtp(phoneNumber); // Gửi mã OTP nếu số điện thoại hợp lệ
          setIsOtpSent(true);
        }
      } catch (error) {
        if (error.response && error.response.status === 404) {
          setErrors({
            phoneNumber: "Số điện thoại không tồn tại trong hệ thống.",
          });
        } else {
        ...
        >
          ...
          <div className="text-center px-4">
            ...
            <p className="opacity-75">
              Đừng lo, hãy nhập số điện thoại của bạn để đặt lại mật khẩu. Chúng
        ...
        <div className="col-lg-6 col-md-8 mx-auto bg-body-secondary p-4 py-5 p-sm-5">
          ...
          {!isOtpSent ? (
            ...
            <p className="text-center text-success">
              Xác thực mã OTP thành công! Bạn có thể đặt lại mật khẩu.
            </p>
          )}
