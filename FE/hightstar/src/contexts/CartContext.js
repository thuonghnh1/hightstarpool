...
export const CartProvider = ({ children }) => {
  ...
  // Tải giỏ hàng từ localStorage khi người dùng đăng nhập
  useEffect(() => {
    if (user && user.userId) {
      const storedCart =
      ...
      setShoppingCartItems([]); // Xóa giỏ hàng khi đăng xuất
    }
  }, [user]);

  // Lưu giỏ hàng vào localStorage bất cứ khi nào giỏ hàng thay đổi
  useEffect(() => {
    if (user && user.userId) {
  ...
  const clearCart = () => {
    ...
    if (user && user.userId) {
      localStorage.removeItem(`shoppingCartItems_${user.userId}`);
    }
