...
const TransactionHistory = () => {
  ...
  return (
    <Container className="mt-5 p-5 bg-light text-dark" fluid>
                          ...
                          if (transaction.transactionDate) {
                            try {
                              ...
                              if (typeof transaction.transactionDate === 'string') {
                                ...
                                parsedDate = parse(transaction.transactionDate, 'dd/MM/yyyy HH:mm:ss', new Date());

                                // Kiểm tra xem parse có thành công không
                                if (!isValid(parsedDate)) {
                                  console.error("Ngày không hợp lệ sau khi parse:", transaction.transactionDate);
                                }
                              } else if (typeof transaction.transactionDate === 'number') {
                                // Giả sử là timestamp (số mili giây)
                                parsedDate = new Date(transaction.transactionDate);
                              } else {
                              ...
                              console.error("Lỗi khi phân tích ngày:", error, transaction.transactionDate);
                            }
                          }
